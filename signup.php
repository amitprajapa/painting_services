...
<html>
    <head>
        <title>Signup - Painting Services</title>
...
if(isset($_POST['signup']))
{
    ...
    {
        $error[]='Pleas Enter Name Using Atlest 3 Characters';
    }
    if(strlen($firstname)>20)
    { // max
        $error[]='First Name Max lent 20 Not allowed';
    }
    
    if(!preg_match("/^[A-Za-z _]*[A-Za-z ]+[A-Za-z _]*$/", $firstname))
    {
        $error[] = 'Invalid Entry First Name. Please Enter 
        letters without any Digit or special symbols like 
        ( 1,2,3#,$,%,&,*,!,~,`,^,-,)';
} 
...
$sql="select * from users where (username='$username' or email='$email');";
     ...
     if(!isset($error))
     {
         ...
         ('','$firstname','$lastname','$username','$email','$password','$date')");
...
<?php if(isset($done)) 
    ...
    <img firstname src="logo.png" alt="" class="logo img-fluid">
        <form action="" method="POST">
        <div class="form-group">
            <label class="label_text">First Name</label>
            <input type="text" class="form-control" name="firstname" 
            value="<?php if(isset($error)){ echo $_POST['firstname'];}?>" required="">
        </div> 
        <div class="form-group">
            <label class="label_text">Last Name</label>
            <input type="text" class="form-control" name="lastname" 
            value="<?php if(isset($error)){ echo $_POST['lastname'];}?>" required="">
        </div> 
        <div class="form-group">
